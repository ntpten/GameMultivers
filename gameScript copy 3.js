...
function setup() {
  ...
  for (var i = 0; i < numObstacles; i++) {
    var obstacle = new Sprite(scene, "images/obstacle.png", 60, 310); // ปรับขนาดของภาพให้แสดงเต็ม
    // ตั้งตำแหน่ง X ของ obstacle ให้ติดขอบขวา และตั้งตำแหน่ง Y ให้ห่างจากขอบล่าง
    obstacle.setPosition(
      SCENE_WIDTH - obstacle.width * 2.5,
      SCENE_HEIGHT - (obstacle.height / 1.75) * 0.5 - i * gap
    ); // ติดขอบขวาและห่างจากขอบล่าง
    obstacle.setAngle(-360); // หมุนวัตถุ 90 องศา ให้แสดงในแนวนอน
    obstacle.setSpeed(5); // ตั้งความเร็ว
    obstacle.setDX(-15); // กำหนดความเร็วในการเคลื่อนที่ทางแนวนอนให้เป็นค่าลบ (ไปทางซ้าย)
...
function update() {
  ...
  for (var i = 0; i < obstacles.length; i++) {
    ...
    // เมื่อ obstacle เคลื่อนที่ออกจากจอไปทางซ้าย ให้สร้างตัวใหม่
    if (obstacles[i].x < -obstacles[i].width) {
      // ลบ obstacle ตัวเก่า
      scene.removeSprite(obstacles[i]);

      // สร้าง obstacle ตัวใหม่ที่ตำแหน่งขวาของหน้าจอ
      var newObstacle = new Sprite(scene, "images/obstacle.png", 60, 310);
      newObstacle.setPosition(
        SCENE_WIDTH,
        SCENE_HEIGHT - (newObstacle.height / 1.75) * 0.5 - i * 170
      ); // ตั้งตำแหน่งใหม่ที่ขอบขวา
      newObstacle.setAngle(-360); // หมุนวัตถุ 90 องศา ให้แสดงในแนวนอน
      newObstacle.setSpeed(5); // ตั้งความเร็ว
      newObstacle.setDX(-5); // กำหนดความเร็วในการเคลื่อนที่ทางแนวนอนให้เป็นค่าลบ (ไปทางซ้าย)
      newObstacle.setDY(0); // ไม่ให้มันเคลื่อนที่ในแนวตั้ง
      obstacles[i] = newObstacle; // เพิ่ม obstacle ตัวใหม่เข้าไปใน array obstacles
    }
  }
}
